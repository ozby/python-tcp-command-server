...
@pytest.mark.asyncio
async def test_create_discussion(discussion_service) -> None:
...
@pytest.mark.asyncio
async def test_create_reply(discussion_service) -> None:
    discussion_id = await discussion_service.create_discussion(
...
@pytest.mark.asyncio
async def test_list_discussions(discussion_service) -> None:
    ...
    test1_discussions = await discussion_service.list_discussions(reference_prefix="test1")
...
@pytest.mark.asyncio
async def test_get_discussion(discussion_service) -> None:
    discussion_id = await discussion_service.create_discussion(
